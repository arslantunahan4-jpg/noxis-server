// Noxis Streaming Worker
// Features: Universal Proxy, M3U8 Rewrite, Adaptive Master Playlist, Diziyou resolve fallback
...
// URL'nin zaten worker tarafından proxy'lenmiş olup olmadığını kontrol et
// Bu, double-wrapping (çift sarmalama) sorununu önler
const isAlreadyProxied = (targetUrl) => {
  const lower = String(targetUrl || '').toLowerCase();
  return lower.includes('mode=proxy') || lower.includes('workers.dev');
};

const buildProxyUrl = (workerUrl, targetUrl, referer) => {
  // Zaten proxy'lenmiş URL'leri tekrar sarmalama
...
// Vidmody segment dosyalarını tanı (seg-X-v1-a1.gif gibi)
const isVidmodySegment = (targetUrl) => {
  return /\/seg-[^/]+\.gif(\?|$)/i.test(targetUrl);
};

// URL ve content-type'a göre playlist olabilecek yanıtları tespit et
// NOT: Bu fonksiyon "kesinlikle playlist" demez, "playlist OLABİLİR" der.
// Proxy mode'da gerçek content doğrulaması ayrıca yapılır.
const isLikelyPlaylist = (targetUrl, contentType) => {
  const lowerUrl = targetUrl.toLowerCase();
  const lowerType = (contentType || '').toLowerCase();

  // Bilinen binary segment uzantıları - bunlar kesinlikle playlist değil
  if (/\.(ts|m4s|mp4|aac|jpg|jpeg|png|webp|vtt|srt)(\?|$)/i.test(targetUrl)) {
    return false;
  }

  // Vidmody seg-*.gif dosyaları binary segment, playlist değil
  if (isVidmodySegment(targetUrl)) {
    return false;
  }

  // Content-Type kesin olarak mpegurl ise playlist
  if (lowerType.includes('mpegurl') || lowerType.includes('application/x-mpegurl')) {
    return true;
  }

  // .m3u8 uzantısı kesin playlist
  if (lowerUrl.includes('.m3u8')) {
    return true;
  }

  // Vidmody index-*.gif dosyaları playlist (sub-playlist)
  // Dikkat: Sadece "index-" ile başlayan .gif dosyaları playlist
  if (/\/index[^/]*\.gif(\?|$)/i.test(targetUrl)) {
    return true;
  }
...
const validateSubtitleTrack = async (subtitleUrl, referer) => {
  ...
  // Zaten proxy'lenmiş URL ise gerçek URL'yi çıkar
  let actualUrl = subtitleUrl;
  try {
    const parsed = new URL(subtitleUrl);
    const innerUrl = parsed.searchParams.get('url');
    if (innerUrl) actualUrl = innerUrl;
  } catch {}

  try {
    const response = await fetch(actualUrl, {
      method: 'GET',
      headers: createUpstreamHeaders(new Request(actualUrl), referer, true)
    });
    ...
    const firstAsset = findFirstPlaylistAsset(text, actualUrl);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('#EXT-X-MEDIA:TYPE=AUDIO')) {
      const rewritten = line.replace(/URI="([^"]+)"/, (match, uri) => {
        ...
        return `URI="${buildProxyUrl(workerUrl, absUri, referer)}"`;
      });
    ...
    if (trimmed.startsWith('#EXT-X-STREAM-INF:')) {
      ...
      if (nextLine && !nextLine.startsWith('#')) {
        ...
        if (absUri.includes('index-v1-a1')) {
          ...
          rewrittenLines.push(buildProxyUrl(workerUrl, trUrl, referer));
          ...
          rewrittenLines.push(buildProxyUrl(workerUrl, enUrl, referer));
        } else {
          // Standard rewrite for single variant
          rewrittenLines.push(line);
          rewrittenLines.push(buildProxyUrl(workerUrl, absUri, referer));
        }
...
// Sub-playlist'lerdeki relative URL'leri proxy URL'lerine çevir
// Content'in gerçekten #EXTM3U olduğu doğrulandıktan sonra çağrılır
const rewriteSubPlaylist = (content, targetUrl, workerUrl, referer) => {
  ...
  for (const line of lines) {
    ...
    // Boş satır veya HLS tag'i - olduğu gibi bırak
    if (!trimmed || trimmed.startsWith('#')) {
      rewrittenLines.push(line);
      continue;
    }

    // URL satırı - mutlak URL'ye çevir ve proxy ile sarmala
    const absoluteUrl = trimmed.startsWith('http')
      ? trimmed
      : new URL(trimmed, baseUrl).href;

    rewrittenLines.push(buildProxyUrl(workerUrl, absoluteUrl, referer));
  }
...
const resolveDiziyouSource = async ({ request, workerUrl, title, season, episode, originalTitle }) => {
  ...
  for (const currentTitle of titlesToTry) {
    try {
      ...
      hasDub = await probeAsset(dubUrl, DIZIYOU_REFERER);
...
export default {
  async fetch(request) {
    ...
    const isVsRequest = (url.pathname.toLowerCase().startsWith('/vs/') || 
                        ...
                        mode !== 'proxy' && mode !== 'diziyou-resolve';
    ...
    if (isVsRequest) {
      ...
      try {
        ...
        // İçerik gerçekten bir HLS manifest mi kontrol et
        if (!content.trim().startsWith('#EXTM3U')) {
          return new Response('Vidmody: Geçerli bir HLS manifest döndürülmedi', { 
            status: 502, 
            headers: corsHeaders 
          });
        }
    ...
    if (isProxyRequest(url)) {
      ...
      try {
        ...
        // Playlist olabilecek bir yanıt mı kontrol et (URL + Content-Type bazlı)
        if (isLikelyPlaylist(targetUrl, contentType)) {
          // Response'u klonla - eğer playlist değilse binary olarak stream edeceğiz
          const clonedResponse = response.clone();
          const text = await response.text();
          const trimmedText = text.trim();

          // Gerçekten bir HLS playlist mi? (#EXTM3U ile başlamalı)
          if (trimmedText.startsWith('#EXTM3U')) {
            // Evet, gerçek bir playlist - URL'leri yeniden yaz
            const rewritten = rewriteSubPlaylist(text, targetUrl, workerUrl, referer);
          ...
          // Playlist değil - klonlanmış response'u binary olarak stream et
          const fallbackHeaders = new Headers(clonedResponse.headers);
          Object.entries(corsHeaders).forEach(([key, value]) => {
        ...
        // Playlist değil - binary stream (segment, altyazı vb.)
        ...
        // Vidmody .gif segment dosyaları için doğru Content-Type ayarla
        if (isVidmodySegment(targetUrl) && !contentType.toLowerCase().includes('video/') && !contentType.toLowerCase().includes('audio/')) {
          newHeaders.set('Content-Type', 'video/mp2t');
        }
    ...
    return new Response('Noxis Worker Active. Use mode=master, mode=proxy, or mode=diziyou-resolve.', {
      headers: corsHeaders
    });
  }
};
