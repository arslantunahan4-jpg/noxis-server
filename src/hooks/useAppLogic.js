...
// --- API UTILS ---
import { MOVIE_CATEGORY_DEFINITIONS, TV_CATEGORY_DEFINITIONS } from '../services/tmdb';
const SERVER_URL = getApiBaseUrl();

export const fetchTMDB = async (endpoint) => {
    try {
        const res = await fetch(`${SERVER_URL}/api/tmdb?endpoint=${encodeURIComponent(endpoint)}`);
...
export { MOVIE_CATEGORY_DEFINITIONS, TV_CATEGORY_DEFINITIONS };
            ...
            validFocusables.forEach(el => {
                ...
                if (isCandidate) {
                    ...
                    if (direction === 'left' || direction === 'right') {
                        dist = Math.abs(c1.x - c2.x) + (Math.abs(c1.y - c2.y) * 4); // Penalize vertical diff
                    } else {
